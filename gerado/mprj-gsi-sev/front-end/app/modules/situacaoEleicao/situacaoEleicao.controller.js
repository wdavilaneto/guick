...
angular.module('sevApp')
    .controller('situacaoEleicaoController', situacaoEleicaoController);

    situacaoEleicaoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'situacaoEleicaoService' , 'eleicaoService'];

    function situacaoEleicaoController($scope, $stateParams , $modal, $location, jsog, situacaoEleicaoService , eleicaoService ) {
        ...
        $scope.gridOptions = situacaoEleicaoService.createGridOptions( true);
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    situacaoEleicaoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                situacaoEleicaoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createSituacaoEleicaoModal.html', controller: 'ModalSituacaoEleicaoController as vm', size: size,
                resolve: {
        ...
        if (!vm.editMode) {
            situacaoEleicaoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
                $scope.gridOptions.totalItems = $scope.resultPage.totalElements;
            });
