...
angular.module('sevApp')
    ...
    eventoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'eventoService' , 'arquivoRelatorioService', 'eleicaoService', 'tipoEventoService'];

    function eventoController($scope, $stateParams , $modal, $location, jsog, eventoService , arquivoRelatorioService, eleicaoService, tipoEventoService ) {
        ...
        $scope.gridOptions = eventoService.createGridOptions(  false );
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    eventoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                eventoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createEventoModal.html', controller: 'ModalEventoController as vm', size: size,
                resolve: {
    ...
    if (!vm.editMode) {
            eventoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
                $scope.gridOptions.totalItems = $scope.resultPage.totalElements;
            });
