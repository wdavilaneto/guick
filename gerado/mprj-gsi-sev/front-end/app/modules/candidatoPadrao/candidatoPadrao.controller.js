...
angular.module('sevApp')
    .controller('candidatoPadraoController', candidatoPadraoController);

    candidatoPadraoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'candidatoPadraoService' , 'candidatoService'];

    function candidatoPadraoController($scope, $stateParams , $modal, $location, jsog, candidatoPadraoService , candidatoService ) {
        ...
        $scope.gridOptions = candidatoPadraoService.createGridOptions(  false );
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    candidatoPadraoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                candidatoPadraoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createCandidatoPadraoModal.html', controller: 'ModalCandidatoPadraoController as vm', size: size,
                resolve: {
    ...
    if (!vm.editMode) {
            candidatoPadraoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
            });
        }
