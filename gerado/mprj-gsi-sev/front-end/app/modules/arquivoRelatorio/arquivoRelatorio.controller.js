...
angular.module('sevApp')
    .controller('arquivoRelatorioController', arquivoRelatorioController);

    arquivoRelatorioController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'arquivoRelatorioService' , 'eleicaoService', 'eventoService', 'tipoRelatorioService'];

    function arquivoRelatorioController($scope, $stateParams , $modal, $location, jsog, arquivoRelatorioService , eleicaoService, eventoService, tipoRelatorioService ) {
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    arquivoRelatorioService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                arquivoRelatorioService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createArquivoRelatorioModal.html', controller: 'ModalArquivoRelatorioController as vm', size: size,
                resolve: {
        ...
        if (!vm.editMode) {
            arquivoRelatorioService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
                $scope.gridOptions.totalItems = $scope.resultPage.totalElements;
            });
