...
angular.module('sevApp')
    ...
    resultadoApuracaoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'resultadoApuracaoService' , 'candidatoService'];

    function resultadoApuracaoController($scope, $stateParams , $modal, $location, jsog, resultadoApuracaoService , candidatoService ) {
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    resultadoApuracaoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                resultadoApuracaoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createResultadoApuracaoModal.html', controller: 'ModalResultadoApuracaoController as vm', size: size,
                resolve: {
    ...
    if (!vm.editMode) {
            resultadoApuracaoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
            });
        }
