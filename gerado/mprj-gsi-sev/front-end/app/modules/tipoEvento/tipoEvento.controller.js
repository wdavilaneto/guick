...
angular.module('sevApp')
    ...
    tipoEventoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'tipoEventoService' , 'eventoService'];

    function tipoEventoController($scope, $stateParams , $modal, $location, jsog, tipoEventoService , eventoService ) {
        ...
        $scope.gridOptions = tipoEventoService.createGridOptions( true);
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    tipoEventoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                tipoEventoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createTipoEventoModal.html', controller: 'ModalTipoEventoController as vm', size: size,
                resolve: {
    ...
    if (!vm.editMode) {
            tipoEventoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
            });
        }
