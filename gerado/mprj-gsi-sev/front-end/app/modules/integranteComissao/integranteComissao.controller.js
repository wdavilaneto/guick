...
angular.module('sevApp')
    .controller('integranteComissaoController', integranteComissaoController);

    integranteComissaoController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'integranteComissaoService' , 'eleicaoService', 'tipoIntegranteComissaoService'];

    function integranteComissaoController($scope, $stateParams , $modal, $location, jsog, integranteComissaoService , eleicaoService, tipoIntegranteComissaoService ) {
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    integranteComissaoService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                integranteComissaoService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createIntegranteComissaoModal.html', controller: 'ModalIntegranteComissaoController as vm', size: size,
                resolve: {
        ...
        if (!vm.editMode) {
            integranteComissaoService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
                $scope.gridOptions.totalItems = $scope.resultPage.totalElements;
            });
