...
angular.module('sevApp')
    ...
    tipoParametroController.$inject = ['$scope', '$stateParams', '$modal', '$location', 'jsog', 'tipoParametroService' , 'parametroService'];

    function tipoParametroController($scope, $stateParams , $modal, $location, jsog, tipoParametroService , parametroService ) {
        ...
        $scope.gridOptions = tipoParametroService.createGridOptions( true);
        ...
        function search(page) {
            ...
            if (vm.textSearchActive) {
                    tipoParametroService.searchText( {'content': vm.searchField.text ,'pagination':page} , function (data) {
                ...
                tipoParametroService.search( {'content':vm.entity,'pagination':$scope.pagination.getPageRequest()} , function (data) {
                    $scope.resultPage = data;
                    try {
                        $scope.resultPage.content = jsog.decode(data.content);
                    } catch (exception) {
        ...
        function open(size) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'createTipoParametroModal.html', controller: 'ModalTipoParametroController as vm', size: size,
                resolve: {
        ...
        if (!vm.editMode) {
            tipoParametroService.findAll( $scope.pagination.getPageRequest() , function (data) {
                $scope.resultPage = data;
                $scope.resultPage.content = jsog.decode(data.content);
                $scope.gridOptions.totalItems = $scope.resultPage.totalElements;
            });
